...
async function run() {
  ...
  global.fetch = async (url, options = {}) => {
    ...
    return { ok: true, status: 200, json: async () => ({ ok: true, leadId: 'lead-fanout' }) };
  };
  ...
  try {
    await withEnv({
      ...
      assert.strictEqual(response.body.crm, 'saved');
      assert.match(response.body.reference, /^[0-9a-f-]{36}$/);
